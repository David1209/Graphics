...
ray_trace(void)
{
    ...
    // calculate the width and hight of a pixel
    pixelw = image_plane_width / framebuffer_width;
    pixelh = image_plane_height / framebuffer_height;
    ...
    for (j = 0; j < framebuffer_height; j++)
    {
        for (i = 0; i < framebuffer_width; i++)
        {
            if(do_antialiasing)
            {
                ...
                ti = -image_plane_width / 2 + i * pixelw - pixelw * 0.5;
                ...
                for(k = 0; k < 2; k++)
                {
                    for(l = 0; l < 2; l++)
                    {
                        ti = -image_plane_width / 2 + i * pixelw - pixelw * 0.5;
                        ti += pixelw * 0.5 * l;
                        tj = image_plane_height/ 2 - j * pixelh - pixelh * 0.5;
                        tj += pixelh * 0.5 * k;
                        direction = v3_add(forward_vector, v3_add(v3_multiply(right_vector, ti), v3_multiply(up_vector, tj)));
